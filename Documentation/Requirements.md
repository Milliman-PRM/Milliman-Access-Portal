...
A role indicated in parentheses below is the authorization requirement for the page view or action.
...
- MAP must limit the user's visibility of and access to only content items that are authorized for access by the user.
- The application landing page must display a view of the content items that the authenticated user is authorized to access.
  ...
  - The link must function for content served from any host that may or may not be colocated with the MAP infrastructure, provided that the content server is properly accessible to the user.
...
- MAP must perform without noticeable performance degradation under normal workflow load of 30 concurrent users.  TODO: What is the right number?
