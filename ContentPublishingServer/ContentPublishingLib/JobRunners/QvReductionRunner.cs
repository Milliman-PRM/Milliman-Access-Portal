    ...
    public class QvReductionRunner : RunnerBase
    {
        ...
        private async Task RunQdsTask(DocumentTask DocTask)
        {
            ...
            try
            {
                ...
                {
                    throw new ApplicationException($"QDS task status {Status.General.Status.ToString()} after task {TaskIdGuid.ToString("D")}:{Environment.NewLine}{Status.Extended.LastLogMessages}");
                }
            }
            finally
            {
