...
export class SelectionsPanel extends React.Component<SelectionsPanelProps> {
  ...
  private renderFormSection() {
    const { isMaster, title, subtitle, isEditableEligible, isEditable, isSuspended, loading } = this.props;
    return loading
      ? <ColumnSpinner />
      : (
        <div className="admin-panel-form" style={{ display: 'flex', flex: 'auto' }}>
          <div className="admin-panel-content-container">
            ...
            >
              ...
              {isEditableEligible && <Toggle
                label={'Editable'}
                checked={isEditable}
                onClick={() => this.props.onIsEditableChange(!isEditable)}
              />
              }
              <Toggle
