    ...
    public static class MockMapDbContext
    {
        ...
        private static List<ApplicationRole> GetSystemRolesList()
        {
            lock (LockObject)
            {
                ...
                {
                    ApplicationRole NewRole = new ApplicationRole { Id = TestUtil.MakeTestGuid((int)Role), RoleEnum = Role, Name = Role.ToString(), NormalizedName = Role.ToString().ToUpper(), DisplayName = Role.GetDisplayValueString() };
