...
namespace ContainerReverseProxy
{
    public class HttpRequestUtils
    {
        internal static string? GetContentTokenForRequest(HttpRequest request, string baseUrl, string contentTokenName, List<string> configuredTokens)
        {
            ...
            string? originHeaderToken = request.Headers.Origin.Any() ? ContentTokenFromHostString(new Uri(request.Headers.Origin).Host, baseUrl) : null;
            string? refererHeaderToken = request.Headers.Referer.Any() ? ContentTokenFromHostString(new Uri(request.Headers.Referer).Host, baseUrl) : null;
            string? originQueryToken = request.Headers.Origin.Any() ? ContentTokenFromUriQuery(new Uri(request.Headers.Origin), contentTokenName) : null;
            string? refererQueryToken = request.Headers.Referer.Any() ? ContentTokenFromUriQuery(new Uri(request.Headers.Referer), contentTokenName) : null;
